...
async def run_server_sse(sqlite_file: str, metadata_yaml_file: str | None = None, host: str = "localhost", port: int = 3001):
    """Run the server with SSE transport over HTTP using a simple HTTP server"""
    try:
        import uvicorn
        ...
        raise ImportError("SSE transport requires 'uvicorn' and 'fastapi'. Install with: uv add uvicorn fastapi")
    ...
    app = FastAPI(title="MCP SQLite Server", description="MCP SQLite Server with SSE transport")
    
    async def sse_stream(request: Request) -> AsyncGenerator[str, None]:
        """Generate SSE events for MCP communication"""
        try:
            # This is a simplified SSE implementation
            # For a proper implementation, you'd need full MCP protocol handling
            yield f"data: {json.dumps({'type': 'hello', 'message': 'MCP SQLite Server Connected'})}\n\n"
    ...
    async def root():
        ...
        return {
            ...
            "metadata": metadata_yaml_file,
            "sse_endpoint": f"http://{host}:{port}/sse"
        }
    
    print(f"🚀 MCP SQLite server starting on http://{host}:{port}")
    print(f"📁 Database: {sqlite_file}")
    if metadata_yaml_file:
        print(f"📋 Metadata: {metadata_yaml_file}")
    print(f"🔗 SSE Endpoint: http://{host}:{port}/sse")
    print(f"ℹ️  Server Info: http://{host}:{port}")
...
def main_cli():
    ...
    if args.transport == "sse":
        anyio.run(run_server_sse, args.sqlite_file, args.metadata, args.host, args.port)
    else:
